...
class TrainSetLoader(Dataset):
    ...
    def copy_target(self, image, mask):
        w, h = image.size
        # print('w')
        # print(w)
        # print('h')
        # print(h)
        # image = np.array((image > 0).cpu()).astype('int64')
        image = np.array(image)
        # image = np.reshape(image, (256, 256))
        mask = np.array(mask)
        # mask = np.reshape(mask, (256, 256))

        image_ = measure.label(image, connectivity=2)
        coord_image = measure.regionprops(image_)
        mask_ = measure.label(mask, connectivity=2)
        coord_mask = measure.regionprops(mask_)

        # for K in range(len(coord_image)):
        #     area_image = np.array(coord_image[K].area)
        image_return = np.copy(image)
        mask_return = np.copy(mask)
        # 图
        for coord in coord_mask:
            ...
            for _ in range(2):
                # 点
                copy_point_x = random.randint(0, h - (coord.bbox[2] - coord.bbox[0]) - 1)
                copy_point_y = random.randint(0, w - (coord.bbox[3] - coord.bbox[1]) - 1)
                # print('copy_point_x')
                # print(copy_point_x)
                # print('copy_point_y')
                # print(copy_point_y)
                for point in coord.coords:
                    # print('point')
                    # print(point)
                    offset_x = point[0] - coord.bbox[0]
                    offset_y = point[1] - coord.bbox[1]
                    image_return[copy_point_x + offset_x, copy_point_y + offset_y] = image[point[0], point[1]]
                    # image_return[] = image[]
                    mask_return[copy_point_x + offset_x, copy_point_y + offset_y] = mask[point[0], point[1]]
                    # mask_return[copy_point_x+offset_x] = mask[point[0]]
                    # mask_return[copy_point_y+offset_y] = mask[point[1]]
        image_return = Image.fromarray(image_return)
        mask_return = Image.fromarray(mask_return)
        return image_return, mask_return
    ...
    def __getitem__(self, idx):
        ...
        # add 2023.2.2
        # img, mask = self.copy_target(img, mask)
...
class TestSetLoader(Dataset):
    ...
    def __init__(self, dataset_dir, img_id,transform=None, base_size=256, crop_size=256,
                 suffix='.png'):
